...
// this.setState(newState, () => console.log(updatedState))
// const [state, setState] = useState(initialState);

// setState(nextState)
// state -> nextState 바뀐 다음에 바로 콜백 함수를 실행시키려면?
// useEffect()를 사용하면 setState를 대체할 수 있다.
...
function Form() {
  ...
  // 상태 업데이트 요청(트리거)
  // 리액트 컴포넌트 (리-렌더)
  // orderState 상태가 업데이트(리-렌더) 된 이후 콜백 함수 실행
  useEffect(() => {
    // orderState가 업데이트 된 마지막 스냅샷
    // console.log(orderState);
    if (orderState.toppings.length > 0) {
      console.log('!');
      // 토핑이 하나라도 추가된다면?
      formRef.current.querySelectorAll('input + label').forEach((label) => {
  ...
  const handleChangeAllToppings = () => {
    // 다음 상태 값
    // const { checked } = e.target; // true

    // 현재(업데이트 이전 시점) 상태 값
    const isChecked = orderState.isAllToppings; // false -> true

    console.log(`현재 상태 값: isChecked = ${isChecked}`);
    // console.log(`다음 상태 값: checked = ${checked}`);

    // 논리적 흐름의 순서상
    // false -> true 변경
    const nextOrderState = {
      ...orderState,
      isAllToppings: !isChecked /* 현재 체크되지 않았다면 체크해라 */,
      toppings: !isChecked /* 현재 체크되지 않았다면 토핑을 모두 채워라 */
        ? PIZZA.toppings
        : [],
    };
  ...
  return (
    <form ref={formRef} onSubmit={handleOrder} onReset={handleCancel}>
      <h3>피자 타입을 선택하세요.</h3>
